...
namespace QuantConnect.Data.Custom.Tiingo
{
    ...
    public class TiingoNewsData : IndexedBaseData
    {
        /// <summary>
        ...
        public override BaseData Reader(SubscriptionDataConfig config, string content, DateTime date, bool isLiveMode)
        {
            var data = JsonConvert.DeserializeObject<List<TiingoNewsData>>(content,
                new TiingoNewsJsonConverter(config.Symbol, config.ExchangeTimeZone));

            if (isLiveMode)
            {
                // we need to reverse the news data since we request data order by 'CrawlDate'
                // and it uses descending order (can't be changed), newest data first
                data.Reverse();
